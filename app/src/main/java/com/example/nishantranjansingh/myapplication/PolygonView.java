...
public class PolygonView extends FrameLayout {
    ...
    public boolean isValidShape(Map<Integer, PointF> pointFMap) {
        return (pointFMap.size() == 4 && checkFOrPoints(pointFMap));
    }

    private boolean checkFOrPoints(Map<Integer, PointF> pointFMap){
        float width1 =  pointFMap.get(1).x - pointFMap.get(0).x;
        float width2 =  pointFMap.get(3).x - pointFMap.get(2).x;
        float height1 =  pointFMap.get(2).y - pointFMap.get(0).y;
        float height2 =  pointFMap.get(3).y - pointFMap.get(1).y;

        if(width1 >= 5 && width2 >= 5 && height1 >= 5 && height2 >= 5){
            return true;
        }else{
            return false;
        }
